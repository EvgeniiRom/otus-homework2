...
interface LoginAction { type: typeof LOGIN, name: string }
interface LogoutAction { type: typeof LOGOUT }
interface ClickAction { type: typeof CLICK, button: string }
...
export default function reducer(state: UserStatState = initState, action: Action): UserStatState {
    switch (action.type) {
        ...
        case CLICK:
            ...
            return { ...state, buttonClicks: { ...state.buttonClicks, [action.button]: clickCount } }
        default:
            return state
    }
}
